...
ShipJob.Spirit.next.push Page.SpiritFog = class SpiritFog extends Page
  ...
  text: ->"""|| bg="Ship.deckFog"
    -- A grey fog descended over the ship some time in the night, and by the time anyone thought to wake Natalie, the Lapis was becalmed in dim water. The lightlessness dragged at spirits and numbing thoughts.
  ...
  ||
    #{@Nat.image 'serious', 'center'}
    --> Drooping sails and silent waves suggested that the Lapis wasn't going anywhere soon, so Natalie didn't bother trying to harangue enthusiasm into them. There was magic at work here.

  ||
    -- #{q}Come out, come out!</q> Her voice echoed, as though the world itself were small.

  ||
    --> No response. Time to do things the hard way then, if whatever was casuing this didn't want to talk.
  ...
  ||
    ...
    #{q}You stopped my ship and bewitched my crew. Fiesty doesn't even begin to cover it.</q> She poked #{Math.choice g.crew} who was standing motionless, drooling slightly. #{q}Please explain.</q>
...
ShipJob.Spirit.next.push Page.SpiritFish = class SpiritFish extends Page
  text: ->"""|| bg="Ship.deckDay"
    ...
    -- #{q}Nets out! Lower the sails. #{Math.choice g.crew}, get everyone on deck. We're going fishing!</q> Sailors scrambled to obey their excited captain as the water around them churned and boiled with fish. For a dozen yards in every direction the water was silver with the glint of scales, a school as large and hungry as Natalie had ever seen. If they'd been properly equiped as a fishing vessel they'd be able to make a month's catch in just a few hours, but even so she couldn't resist the chance to stock up their larder.

  ||
    -- They had only to drop a net overborad before it was full, then pull it aboard to fill a barrel with the flopping, squirming take. They set up a line - catch, behead, scale, gut, drop in a barrel and sprinkle salt over the top.
  ...
  ||
    --> Before anyone could gather weapons, or even formulate a coherent response to the sudden invasion by the oddly non-destructive tentacles (they hadn't even knocked anything over in their questing), the monster's body rose out of the water. A giant face, a wise old man hoary with age, but in green and dripping with sea water stared back at them.

    <q>Why are you hurting my friends?</q> It rumbled in a deep masculine voice, a sad frown rippling across its giant brow.
