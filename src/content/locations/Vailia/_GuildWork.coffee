...
Job.SearchGuildWork.next.push Page.SearchGuildWorkNatalie = class SearchGuildWorkNatalie extends Page
  ...
  <page>
    <text continue><p><q>Well, if you just want something quick, I heard Ben Oakly's been having trouble with higher-end clients, ever since Wend ran off with that UTC trader to Kantis. Did you hear about that fuss?</q></p></text>
  </page>
...
Job.SearchGuildWork.next.push Page.SearchGuildWorkJames = class SearchGuildWorkJames extends Page
  conditions:
    ' ': {is: Officer.James}
    Nat: '|officers|Nat'
  text: ->"""<page bg="#{if g.weather is 'storm' then g.map.Ship.images.deckStorm else g.map.Ship.images.deckDay}">#{@[' '].image 'angry', 'right'}
  ...
  <page>
    #{@Nat.image 'normal', 'left'}
    <text continue><p>#{q}Aww, come on. You're got getting any around here,</q> she pinched his cheek, pulling her hand away before he could swat it, #{q}so...</q></p></text>
  </page>

  <page>
    #{@[' '].image 'upset', 'right'}
    <text><p>#{q}Stop it, Nat, not funny.</q> He swatted her hand away before she could grab his other cheek. She dropped her hand and stepped back, a sign that she was done teasing for the moment.</p></text>
  </page>
  <page>
    #{@[' '].image 'blush', 'right'}
    <text continue><p>#{q}Look, I know you were raised to see it as no big deal, but this is important to me. I'm not going to sleep with anyone just because I want a few coins in my pocket.</q></p></text>
  </page>"""
...
Job.SearchGuildWork.next.push Page.SearchGuildWorkKat = class SearchGuildWorkKat extends Page
  conditions:
    ...
    Nat: '|officers|Nat'
  text: ->"""<page bg="#{if g.weather is 'storm' then g.map.Ship.images.deckStorm else g.map.Ship.images.deckDay}">
    <text><p>#{q @[' ']}I told you a long time ago I don't want to be a whore,</q> Kat turned her back on Natalie, leaning over the edge of the ship.</p></text>
  </page>
  <page>
    #{@Nat.image 'normal', 'left'}
  ...
  <page>
    <text continue><p>#{q @[' ']}Oh, I like sex alright. But doncha'know, I'd rather sleep with you than anyone else,</q> she wiggled her rear, punctuating the motion by winking over her shoulder.</p></text>
  </page>
  ...
  <page>
    <text continue><p>She got a calculating look in her eye for a moment, then turned back to face Natalie fully. #{q @[' ']}And what, hypothetically speaking, would you gain from this?</q></p></text>
  </page>
  <page>
    #{@Nat.image 'normal', 'left'}
  ...
  <page>
    <text continue><p>#{q @[' ']}And why would I pay you, for work I was doing?</q></p></text>
  </page>"""
...
Page.GuildNat.next.push Page.GuildNatBang = class GuildNatBang extends Page
  conditions:
    ' ': {}
  text: ->"""<page bg="#{portNightImage()}">
    <text><p><q>I have just the thing. Party of six, looking for a cumslut for the night. All men, though they don't look like the type of play too rough. All my girls were already too tired for that when they came in, though I have a private room. Interested?</q></p></text>
  </page>"""
...
Page.GuildNatBang.next['No'] = Page.GuildNatBangNo = class GuildNatBangNo extends Page
  ...
  text: ->"""<page>
    #{@[' '].image 'normal', 'left'}
    <text continue><p>#{q}Sorry, I think I'll pass. They sound like delightful people, though, so give them my regards.</q> Though she wasn't particularly sensitive about her body or sex in general, this didn't sound like the sort of thing she wanted to do tonight.</p></text>
  </page>
  <page>
...
Page.GuildNatBang.next['Sure'] = Page.GuildNatBangSure = class GuildNatBangSure extends Page
  ...
  text: ->"""<page>
    #{@[' '].image 'normal', 'left'}
    <text continue><p>#{q}Sounds good. What do they look like?</q></p></text>
  </page>
  <page>
    <text continue><p><q>Blue uniforms. Can't miss 'em, right in the middle of the lobby, sitting and gabbing all night long. I expect to be paid my share,</q> he waggled a finger at her. She ignored the insinuation and pushed past him. Through the kitchen she went, pausing only to straighten her messy hair at least a little and nodding to another woman resting against one of the counters.</p></text>
  </page>
  <page bg="#{g.map.Vailia.images.tavern}">
    <text><p>The main room was surprisingly busy for such a relatively irreputible business. She spotted her boys easily – as he'd said, hard to miss, in their matching blue uniforms. They looked sharp, despite rough faces. Probably the crew for some important diplomat – a merchant would never waste money outfitting their sailors so, no matter how well off.</p></text>
  </page>
  ...
  <page>
    #{@[' '].image 'normal', 'left'}
    <text><p><q>Hey yourself, good looking. You here for our entertainment?</q></p>
    <p>#{q}I'm here to have some fun. If you think it'll be entertaining, who am I to complain?</q> She smirked and rolled her shoulders sensuously. #{q}Shall we find a room?</q></p></text>
  </page>"""
...
Page.GuildNat.next.push Page.GuildNatDom = class GuildNatDom extends Page
  ...
  <page>
    <text continue><p><q>Oh, they want something alright. They're just too scared to ask for it,</q> Ben gave her a leer. She swatted him on the shoulder.</p></text>
  </page>"""
  next: Page.userChoice
  @next: {}
...
Page.GuildNatDom.next['No'] = Page.GuildNatDomNo = class GuildNatDomNo extends Page
  ...
  text: ->"""<page>
    #{@[' '].image 'normal', 'left'}
    <text continue><p>#{q}You keep your mind out of my pants, and let them make up their own minds. I'm not in the business of convincing people to do things they'll regret.</q></p></text>
  </page>
  <page>
...
Page.GuildNatDom.next['Sure'] = Page.GuildNatDomSure = class GuildNatDomSure extends Page
  ...
  <page>
    #{@[' '].image 'excited', 'left'}
    <text><p>Natalie quirked an eyebrow and smirked. #{q}Not out, to my room. You've picked me.</q></p></text>
  </page>
  <page>
    <text continue><p>The strawberry girl opened her mouth, closed it again, leaned in to whisper something to her friend with a blush that quickly spread to the freckled face, who whispered something back. Natalie smiled to herself – she'd judged them right, a thought confirmed by their next words. Or lack thereof.</p></text>
  </page>
  <page>
...
Page.GuildNatDomSure::next = Page.GuildNatDomSure2 = class GuildNatDomSure2 extends Page
  ...
  <page>
    #{@[' '].image 'blush', 'left'}
    <text continue><p>She pushed him away gently. #{q}They're satisfied. Send my share to the Guild, I'll get it there.</q> Natalie smiled and quirked her head, clapping his shoulder. #{q}Whatever you're thinking, don't try it. Treating me fairly is the cheaper option by far.</q></p></text>
  </page>"""
