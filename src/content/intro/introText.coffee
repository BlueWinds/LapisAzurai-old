Page.Intro = class Intro extends Page
  ...
  text: ->
    hairColor = {}
    for color of Officer.Natalie.colors[3]
    ...
    || class="select-color"
      #{@Nat.normal 'left'}
      -- Grinning at the thought of herself in wrinkles and walking across the deck with a cane – as if! - she brushed #{dropdown hairColor, @Nat.color[3]} hair out of her eyes and examined herself in the mirror. #{dropdown eyeColor, @Nat.color[2]} eyes, still sleepy. She slapped her #{dropdown skinColor, @Nat.color[0]} cheeks and stuck her tongue out at the reflection. There were old sailors and old captains, good enough or lucky enough, and she was going to be one of them. No cane though.

      #{options ['Done']}
""")

    element.find('input').change =>
      # Set Nat's new colors
      checked = $ 'input:checked', element
      @Nat.color = [checked.eq(2).val(),
        'none'
        checked.eq(1).val()
        checked.eq(0).val()
        'none'
      ...
      $('.person', element).replaceWith @Nat.normal('left')
...
""")
    ...
    element.find('input').change =>
      ...
      $('.person', element).last().replaceWith @James.blush 'right', false
