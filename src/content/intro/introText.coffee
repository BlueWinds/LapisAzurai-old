...
Page.Intro2 = class Intro2 extends Page
  ...
  text: -> """<page bg="#{@Ship.images.cabinDay}">
    <text><p>#{q @James}Nat! You ok?</q> James called in on her.</p></text>
  </page>
  <page>
    #{@Nat.image 'normal', 'left'}
  ...
  <page>
    #{@Nat.image 'normal', 'left'}
    <text continue><p>#{q}“It's nothing. How much time do we have?</q> She splashed water on her face, dried it off again with a towel.</p></text>
  </page>
  <page>
  ...
  <page slow bg="#{@Ship.images.deckDay}">
    ...
    <text><p>#{q}I don't believe you,</q> she opened the door, stepping out to join him on deck. #{q}You were rather drunk last night. How are your eyes not red?</q></p></text>
  </page>
  <page bg="#{@Ship.images.deckDay}">
    #{@Nat.image 'excited', 'left'}
    <text><p>#{q}I don't believe you,</q> she opened the door, stepping out to join him on deck. #{q}You were rather drunk last night. How are your eyes not red?</q> Natalie poked James in the forehead, causing him to wince. #{q}I knew it, you just woke up too.</q></p></text>
  </page>
  <page>
    #{@James.image 'blush', 'right'}
    <text continue><p>He shook his head, trying to deny it, but she quirked an eyebrow, and he finally relented with a nod. #{q}Hey, it was our very first night on your brand new ship. A little drinking seemed in order.</q></p></text>
  </page>
  <page>
    #{@Nat.image 'normal', 'left'}
