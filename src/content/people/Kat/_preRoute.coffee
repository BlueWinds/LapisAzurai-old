...
katOptions = [
  ...
  "Any chance I can get my other money back? (<em><span class='business'>+2 business</span> for Natalie</em>)"
  "No thanks, I have better things to do. (<em><span class='happiness'>-3 happines</span> for Kat, <span class='business'>+4 business</span> for Natalie</em>)"
]
...
Page.KatStorm::next = Page.KatStorm2 = class KatStorm2 extends PlayerOptionPage
  ...
  ||
    #{@Nat.normal 'left'}
    --> #{q}That's cream. You don't want to drink it. Here, have a biscuit instead. Tea?</q>
  ...
  ||
    #{@Nat.happy 'left'}
    -- #{q}What? No! I saw you dieing by the side of the road and decided to do something about it. That's all.</q> Natalie felt as though she should be indignant, but really, how many times had she walked past someone in exactly the same situation?
...
Page.KatClothes.next.Angry = Page.KatClothesAngry = class KatClothesAngry extends Page
  ...
  || bg="Ship.cabinDay'
    #{Kat.normalNude 'right'}

  ||
    #{Kat.uncertainNude 'right'}
...
Page.KatClothesLaughing::next = Page.KatClothesLaughing2 = class KatClothesLaughing2 extends Page
  ...
  || speed="slow"
    #{@Kat.embarrassed 'right'}
    --> Slowly, the thief complied.

  ||
    #{@Kat.embarrassedNude 'right'}
    --> Though James looked away, refusing to make eye contact with either woman, plenty of the crew took their chance to gather an eye full. Finally, Kat stood back up, the scene reversed from earlier - one naked woman alone, with everyone else dressed.
  ...
  ||
    #{James.blush 'left'}
    -- Natalie turned, her jacket flashing open with the motion. #{q}Are you going to... um... get dressed, captain?</q>

  ||
    #{@Nat.normalNude 'left'}
    --> #{q}Nah, I'm enjoying the breeze. Besides, it's kind of hot out here, don't you think?</q> She gently shoved Kat's rump with one bare foot, eliciting a squeal of indignation. #{q}Actually, you can have shoes. Punishment is one thing, splinters are another. No socks though. Your ankles are too sexy to cover up.</q>

  ||
    --> <span class="happiness">-2 happiness</span> for James, <span class="happiness">+2</span> for Kat
  """
  apply: ->
    super()
