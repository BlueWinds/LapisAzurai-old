...
Page.MeetKat3.next['Stolen purse?'] = Page.MeetKatMoney = class MeetKatMoney extends Page
  ...
  <page>
    #{@Guard.image 'normal', 'far-right'}
    #{@Kat.image 'sad', 'right'}
    <text continue><p>#{q}No,</q> Kat, finally spoke, tugging at her blouse. It looked both brand new and none too cheap. One of the shoulders was torn where Natalie had grabbed her. She stared at the rip a moment then started to cry silently. She hiccuped as the guard cuffed her ear.</p></text>
  </page>
  <page>
    #{@Kat.image 'sad', 'right'}
    #{@Guard.image 'normal', 'far-right'}
...
Page.MeetKat3.next['Not interested'] = Page.MeetKatNo = class MeetKatNo extends Page
  ...
  text: ->"""<page bg="#{g.location.images.day}">
    #{@Natalie.image 'normal', 'left'}
    <text><p>#{q}No thanks, I have better things to do.</q> She took a moment to count the money in her pouch, steadfastly ignoring her as the thief began to cry. Kat hiccuped as the guard cuffed her ear, and sobbed silently.</p></text>
  </page>
  <page>
    #{@Kat.image 'sad', 'right'}
    #{@Guard.image 'normal', 'far-right'}
